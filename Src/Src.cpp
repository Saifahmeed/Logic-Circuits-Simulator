...
struct OutputInfo {
    ...
    OutputInfo(int t, const string& name, bool val) : time(t), outputName(name), value(val) {}
};
  ...
  circuit_file.seekg(0, ios::beg);
    ...
    while (getline(circuit_file, line)) { // Read each line from the file
      ...
      if (token == "INPUTS:") { // Check if current line specifies inputs section
        ...
        // Check if output name conflicts with input name
        ...
        if (output_count[output_name] > 1) {
            cout<<"Output "<< output_name<<" appears as output for "<< output_count[output_name]<<" different components"<< endl;
            exit(1);
        }
...
void simulate(const vector < Stimulus > & stimuli, map < string, circ_comp > & circuit, unordered_map < string, Component > & components, map < string, bool > inputs, int simulation_time) {
    ...
    for (const auto& stim : stimuli) {
            outputInfos.push_back(OutputInfo(stim.time_stamp_ps, stim.input, stim.logic_value));
    }
    ...
    for (const auto &outputInfo : outputInfos) {
        outputFile << outputInfo.time << ", " << outputInfo.outputName << ", " << outputInfo.value << endl;
    }
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        cerr << "Usage: " << argv[0] << " <library_file> <circuit_file> <stimuli_file>" << endl;
        return 1;
    }
    string libraryFile = argv[1];
    string circuitFile = argv[2];
    string stimuliFile = argv[3];
    int simulation_time=4000;
    unordered_map<string, Component> components = get_component(libraryFile);
    auto [circuit, inputs] = get_circuit(circuitFile);
    vector<Stimulus> stimuli = get_stimulus(stimuliFile);
    circuit_test(circuit, inputs, components);
    evaluate_and_propagate(circuit, components, stimuli);
    test_for_stimlui_error(stimuli, inputs);
    simulate(stimuli, circuit, components, inputs, simulation_time); // Pass the simulation file name to simulate function
    return 0;
}